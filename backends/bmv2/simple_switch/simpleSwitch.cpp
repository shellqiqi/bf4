...
SimpleSwitchBackend::convert(const IR::ToplevelBlock* tlb) {
  ...
  if (!options.psaFile.isNullOrEmpty()) {
    auto structure = new V1ProgramStructure();
    auto parseV1Arch = new ParseV1Architecture(structure);
    auto main = oldtlb->getMain();
    if (!main)
      return; // no main
    main->apply(*parseV1Arch);
    PSAConverter psaConverter(options, refMap, typeMap, v1model, structure);
    psaConverter.convert(oldtlb->getProgram());
    return;
  }
  if (!options.explicitFieldLists.isNullOrEmpty()) {
    auto structure = new V1ProgramStructure();
    auto parseV1Arch = new ParseV1Architecture(structure);
    auto main = oldtlb->getMain();
    if (!main)
    ...
    main->apply(*parseV1Arch);
    PSAConverter psaConverter(options, refMap, typeMap, v1model, structure);
    psaConverter.makeExplicitFieldLists(oldtlb->getProgram());
    return;;
  }
  auto parseV1Arch = new ParseV1Architecture(structure);
