...
class ContainsMethodCalls : public Inspector {
  bool contains = false;
  bool preorder(const IR::Node *) override { return !contains; }
...
public:
  static bool any(const IR::Node *nd) {
    ContainsMethodCalls methodCalls;
    nd->apply(methodCalls);
    return methodCalls.contains;
  }
};
...
const IR::Node* Predication::preorder(IR::IfStatement* statement) {
  ...
  if (ContainsMethodCalls::any(statement->condition) ||
      ContainsMethodCalls::any(statement->ifTrue) ||
      (statement->ifFalse && ContainsMethodCalls::any(statement->ifFalse))) {
      return statement;
  }
