...
SymbolicValue* SymbolicValueFactory::create(const IR::Type* type, bool uninitialized) const {
  if (type->is<IR::Type_Type>())
    type = type->as<IR::Type_Type>().getP4Type();
  ...
  if (type->is<IR::Type_Newtype>()) {
    auto nt = type->to<IR::Type_Newtype>();
    type = nt->type;
    type = typeMap->getTypeType(type, true);
  }
  if (type->is<IR::Type_Typedef>()) {
    auto td = type->to<IR::Type_Typedef>();
    type = td->type;
    type = typeMap->getTypeType(type, true);
  }
  ...
  if (type->is<IR::Type_Enum>() ||
    ...
    auto senum = new SymbolicEnum(type);
    if (!uninitialized)
      senum->state = ScalarValue::ValueState::NotConstant;
    return senum;
  }
  ...
  if (type->is<IR::Type_Parser>() || type->is<IR::Type_Control>() ||
      ...
      type->is<IR::Type_Table>() || type->is<IR::Type_Action>() ||
      type->is<IR::Type_Meter>() || type->is<IR::Type_Counter>())
...
unsigned SymbolicValueFactory::getWidth(const IR::Type* type) const {
  ...
  if (type->is<IR::Type_Bits>())
    return (unsigned)type->to<IR::Type_Bits>()->size;
  if (type->is<IR::Type_Boolean>())
    return 1;
...
bool SymbolicStruct::merge(const SymbolicValue* other) {
  ...
  bool changes = false;
  for (auto f : sv->fieldValue) {
    if (!fieldValue.count(f.first)) {
      BUG("comparing structs of different types");
    }
    auto crt = fieldValue[f.first]->merge(f.second);
    changes = changes || crt;
  }
  return changes;
}
...
bool SymbolicArray::merge(const SymbolicValue* other) {
  ...
  bool changes = false;
  for (unsigned i = 0; i < values.size(); i++) {
    auto crt = values.at(i)->merge(other->to<SymbolicArray>()->get(nullptr, i));
    changes = changes || crt;
  }
  return changes;
}
...
bool SymbolicTuple::merge(const SymbolicValue* other) {
  ...
  bool changes = false;
  for (unsigned i = 0; i < values.size(); i++) {
    auto b = values.at(i)->merge(tpl->get(i));
    changes = changes || b;
  }
  return changes;
}
...
void ExpressionEvaluator::postorder(const IR::Operation_Binary* expression) {
  ...
  BUG_CHECK(l->is<SymbolicInteger>() || l->is<SymbolicBool>(),
            "%1%: expected an SymbolicInteger or SymbolicBool", l);
  BUG_CHECK(r->is<SymbolicInteger>() || l->is<SymbolicBool>(),
            "%1%: expected an SymbolicInteger or SymbolicBool", r);
  if (l->is<SymbolicBool>() && r->is<SymbolicBool>()) {
    auto li = l->to<SymbolicBool>();
    auto ri = r->to<SymbolicBool>();
    if (li->isUninitialized()) {
      ...
      clone->left = new IR::BoolLiteral(li->value);
      clone->right = new IR::BoolLiteral(ri->value);
      DoConstantFolding cf(refMap, typeMap);
      auto result = clone->apply(cf);
      BUG_CHECK(result->is<IR::BoolLiteral>(), "%1%: expected a constant",
                result);
  ...
  } else if (l->is<SymbolicInteger>() && r->is<SymbolicInteger>()) {
    ...
    BUG("different types for %1% %2%", l, r);
  }
}
...
void ExpressionEvaluator::postorder(const IR::Operation_Unary* expression) {
  ...
  if (l->is<SymbolicInteger>()) {
    ...
    typeMap->setType(clone, typeMap->getType(expression));
    DoConstantFolding cf(refMap, typeMap);
    auto result = clone->apply(cf);
    BUG_CHECK(result->is<IR::Constant>(), "%1%: expected a constant", result);
    set(expression, new SymbolicInteger(result->to<IR::Constant>()));
...
void ExpressionEvaluator::postorder(const IR::Operation_Relation* expression) {
  ...
  if (l->is<SymbolicInteger>()) {
    ...
    set(expression, new SymbolicBool(result->to<IR::BoolLiteral>()));
    return;
  } else if (l->is<SymbolicEnum>()) {
    auto lse = l->to<SymbolicEnum>();
    auto rse = r->to<SymbolicEnum>();
    if (expression->is<IR::Equ>()) {
      set(expression, new SymbolicBool(lse->equals(rse)));
    } else {
      BUG("can't handle this relation %1%", expression);
    }
    return;
  }
  BUG("%1%: unexpected type", l);
...
void ExpressionEvaluator::postorder(const IR::Member* expression) {
  ...
  if (type->is<IR::Type_Enum>()) {
    const IR::Path *p = nullptr;
    if (expression->expr->is<IR::PathExpression>()) {
      p = expression->expr->to<IR::PathExpression>()->path;
    } else if (expression->expr->is<IR::TypeNameExpression>()) {
      p = expression->expr->to<IR::TypeNameExpression>()->typeName->path;
    }
    if (p) {
      auto dec = refMap->getDeclaration(p);
      if (dec->is<IR::Type_Enum>()) {
        auto tt = typeMap->getTypeType(dec->getNode(), true);
        auto v =
            tt->to<IR::Type_Enum>()->members.getDeclaration(expression->member);
        CHECK_NULL(v);
        set(expression, new SymbolicEnum(tt, expression->member));
        return;
      }
    }
  } else if (type->is<IR::Type_Error>()) {
    if (expression->expr->is<IR::TypeNameExpression>()) {
      set(expression, new SymbolicEnum(type, expression->member));
      return;
    }
  }
  auto l = get(expression->expr);
  ...
  if (basetype->is<IR::Type_Stack>()) {
    ...
    set(expression, v);
  } else if (basetype->is<IR::Type_Type>()) {
    auto innerType = basetype->to<IR::Type_Type>()->type;
    if (innerType->is<IR::Type_Enum>() || innerType->is<IR::Type_Error>()) {
      auto symenum = factory->create(innerType, false)->to<SymbolicEnum>();
      symenum->assign(new SymbolicEnum(innerType, expression->member));
      set(expression, symenum);
    }
  } else {
...
bool ExpressionEvaluator::preorder(const IR::ArrayIndex* expression) {
  bool lv2 = evaluatingLeftValue;
  evaluatingLeftValue = false;
  visit(expression->left);
  evaluatingLeftValue = lv2;
...
void ExpressionEvaluator::postorder(const IR::PathExpression* expression) {
  ...
  if (decl->is<IR::Declaration_Constant>()) {
    auto declConst = decl->to<IR::Declaration_Constant>();
    if (type->is<IR::Type_Boolean>()) {
      set(expression,
          new SymbolicBool(
              declConst->initializer->to<IR::BoolLiteral>()->value));
    } else {
      if (!value.count(declConst->initializer)) {
        visit(declConst->initializer);
      }
      set(expression, get(declConst->initializer));
    }
    return;
  }
  SymbolicValue *result;
  if (auto tt = type->to<IR::Type_Type>()) {
    if (tt->type->is<IR::Type_Error>() || tt->type->is<IR::Type_Enum>()) {
      result = new SymbolicEnum(type, decl->getName());
    } else {
      BUG("can't handle tt %1%", tt);
    }
  } else if (type->is<IR::Type_Error>() || type->is<IR::Type_Enum>()) {
    result = new SymbolicEnum(type, decl->getName());
  } else {
...
bool ExpressionEvaluator::preorder(const IR::Slice *expression) {
  auto l = expression->getL();
  auto h = expression->getH();
  auto what = expression->e0;
  visit(what);
  auto res = get(what);
  BUG_CHECK(res != nullptr, "%1% cannot evaluate member of slice ", expression);
  if (res->is<SymbolicError>()) {
    set(expression, res->to<SymbolicError>());
  } else {
    BUG_CHECK(res->is<ScalarValue>(),
              "%1% cannot evaluate member of slice. it must be a scalar ",
              what);
    auto scalar = res->to<SymbolicInteger>();
    auto bits = new IR::Type_Bits(h - l + 1, false);
    typeMap->setType(bits, new IR::Type_Type(bits));
    auto val = factory->create(bits, scalar->isUninitialized());
    val->setAllUnknown();
    set(expression, val);
  }
  return false;
...
void ExpressionEvaluator::postorder(const IR::Mux *expression) {
  auto e0 = get(expression->e0)->to<SymbolicBool>();
  auto e1 = get(expression->e1);
  auto e2 = get(expression->e2);
  if (e0->isKnown() && e0->value) {
    set(expression, e1);
  } else if (e0->isKnown() && !e0->value) {
    set(expression, e2);
  } else {
    auto tp = typeMap->getType(expression);
    set(expression, factory->create(tp, false));
  }
}

void ExpressionEvaluator::postorder(const IR::LOr *expression) {
  auto e0 = get(expression->left);
  auto e1 = get(expression->right);
  auto e0s = e0->to<P4::SymbolicBool>();
  auto e1s = e1->to<P4::SymbolicBool>();
  if (e0s && e1s) {
    set(expression, *e0s || e1s);
  } else {
    BUG("something is wrong");
  }
}
void ExpressionEvaluator::postorder(const IR::LAnd *expression) {
  auto e0 = get(expression->left);
  auto e1 = get(expression->right);
  auto e0s = e0->to<P4::SymbolicBool>();
  auto e1s = e1->to<P4::SymbolicBool>();
...
bool ExpressionEvaluator::preorder(const IR::Cast *cast) {
  visit(cast->expr);
  auto ctype = typeMap->getType(cast);
  auto v = get(cast->expr);
  if (v->is<P4::SymbolicBool>()) {
    auto sb = v->to<P4::SymbolicBool>();
    if (sb->state == P4::ScalarValue::ValueState::Constant) {
      BUG_CHECK(ctype->is<IR::Type_Bits>(), "expecting bit<1>, got %1%", ctype);
      if (sb->value) {
        set(cast, new P4::SymbolicInteger(new IR::Constant(ctype, 1)));
      } else {
        set(cast, new P4::SymbolicInteger(new IR::Constant(ctype, 0)));
      }
    }
  } else if (v->is<P4::SymbolicInteger>()) {
    auto si = v->to<P4::SymbolicInteger>();
    if (si->state == P4::ScalarValue::ValueState::Constant) {
      set(cast, new P4::SymbolicInteger(
                    new IR::Constant(ctype, si->constant->value)));
    }
  }
  set(cast, factory->create(ctype, false));
  return true;
}

bool ExpressionEvaluator::preorder(const IR::Declaration_Constant *d) {
  visit(d->initializer);
  return true;
}
